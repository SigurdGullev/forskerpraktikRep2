...
def plot_with_regression_line(df, x_col, y_col, title, scatter_coslor='#8bcfbd', line_color='black', background_color='#e5e5e5'):
    x = df[x_col]
    y = df[y_col]
...
def plot_collider_dag(df, scatter_color='#8bcfbd', line_color='black', background_color='#e5e5e5'):
    ...
    ax.scatter(df, alpha=0.5, color=scatter_color)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    st.pyplot(fig)
